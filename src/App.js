...
// Helper per identificare il dispositivo
    ...
    >
      ...
      {showHelp && (
        <div className="fixed inset-0 bg-slate-900/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-sm w-full overflow-hidden border border-white/20">
            ...
            <div className="p-8 space-y-6">
              <FeatureInfo
                ...
                text="Compila il modulo. Se inserisci una matricola già esistente, Cliente e Dati Gru si compilano automaticamente (campi verdi)."
              />
              <FeatureInfo
                icon={Search}
                color="text-emerald-500"
                title="Ricerca Storico"
                text="Cerca per Cliente o Matricola. Clicca sulla scheda colorata che appare per vedere tutto lo storico e i documenti allegati."
              />
              <button
                onClick={() => setShowHelp(false)}
                className="w-full py-4 bg-blue-700 text-white rounded-2xl font-black text-xs tracking-widest uppercase hover:bg-blue-800 transition-colors"
              >
                Ho capito
              </button>
            </div>
...
function FeatureInfo({ icon: Icon, color, title, text }) {
  return (
    <div className="flex gap-4 items-start">
      <div
        className={`p-3.5 rounded-2xl bg-slate-50 border border-slate-100 shadow-sm ${color}`}
      >
...
function NewEntryForm({
  ...
  // Funzione per resettare i campi macchina
  ...
  const handleCustomerSearchChange = (e) => {
    ...
    // SE IL CAMPO VIENE SVUOTATO, RESETTA TUTTO
    if (val.trim() === "") {
      resetMachineData();
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2 relative" ref={wrapperRef}>
            ...
            {/* SEZIONE GRU DEL CLIENTE SELEZIONATO */}
        ...
        <button
          ...
          className="w-full py-5 bg-blue-700 text-white rounded-2xl font-black text-sm shadow-xl hover:bg-blue-800 hover:-translate-y-0.5 transition-all active:scale-95 disabled:opacity-50 disabled:shadow-none"
        >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-200">
                  ...
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest w-[220px]">
                    Cliente
                  </th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest w-36">
                    Matricola
                  </th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest min-w-[350px]">
                    Lavoro
                  </th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest w-28">
                    Portata
                ...
                {filtered.map((log) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm font-bold text-slate-800 uppercase tracking-tight leading-snug block max-w-[200px]">
                        {log.customer}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-gradient-to-br from-slate-200 to-slate-300 rounded-full flex items-center justify-center text-[10px] font-black text-slate-600 border-2 border-white shadow-sm">
...
function MachineHistoryModal({ machineId, machines, allLogs, onClose }) {
  // Trova la macchina "viva" dallo stato aggiornato
  ...
  const [tab, setTab] = useState("history"); // 'history' o 'docs'
  ...
  const handleDeleteAttachment = async (att) => {
    ...
    try {
      // Nota: per eliminare dallo storage servirebbe il path completo o ref, qui semplifichiamo togliendo solo il link dal DB per ora
      // In un'app reale elimineresti anche l'oggetto dallo storage.
      await updateDoc(
        doc(
          db,
          "artifacts",
    ...
    try {
      await updateDoc(
        ...
        ),
        {
          ...
          dateString: data.dateString, // Allow editing date string directly if needed, usually safer to keep created timestamp
        }
      );
      onClose();
    } catch (e) {
...
function AdminPanel({
  ...
  const [driveBackupStatus, setDriveBackupStatus] = useState("idle");
  ...
  const runFullBackup = () => {
    ...
    a.href = url;
    a.download = `BACKUP_MORA_${new Date().toISOString().split("T")[0]}.csv`;
    setTimeout(() => {
      a.click();
    ...
    try {
      ...
      if (navigator.share) {
        ...
        await navigator.share({
          files: [file],
          title: "Backup Database Mora",
          text: "Rapporti manutenzione aggiornati.",
        });
  ...
  return (
    <div className="space-y-8 animate-in fade-in">
      ...
      {view === "backup" && (
        <div className="max-w-2xl mx-auto animate-in slide-in-from-bottom-4 space-y-6">
          <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-xl text-center space-y-6">
            ...
            <div className="grid gap-4">
              <button
                ...
                className={`w-full py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 shadow-md border-b-4 active:border-b-0 active:translate-y-1 ${
              ...
              >
                {backupStatus === "running" ? (
                  ...
                  <>
                    <CheckCircle2 className="w-4 h-4" /> Esportato!
                  </>
                ) : (
                  <>
                    <Download className="w-4 h-4" /> Download Locale CSV
                  </>
                )}
              </button>

              <button
                onClick={runDriveBackup}
              ...
              >
                {driveBackupStatus === "running" ? (
                  ...
                  <>
                    <CheckCircle2 className="w-4 h-4" /> Inviato!
                  </>
                ) : (
                  <>
                    <Share2 className="w-4 h-4" /> Invia a Google Drive / WA
                  </>
                )}
              </button>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 space-y-2">
              ...
              <p className="text-[8px] text-slate-400 font-medium leading-relaxed italic">
                Il backup Cloud ti permette di condividere il file CSV su
                qualsiasi app o salvarlo nel tuo spazio Drive personale.
              </p>
            </div>
          </div>
        </div>
